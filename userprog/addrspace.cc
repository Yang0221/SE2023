...
#ifdef CHANGED
    BitMap * bitMap = new BitMap((int) MAX_THREAD_NUMBER );
#endif // CHANGED
...
int AddrSpace::AllocateUserStack()
{
    ...
    ASSERT(find!=-1);
    currentThread->setId(find);
    return (numPages*PageSize) - (find*PageSize*2) ;
}
//deallocate the user stack
void AddrSpace::DeallocateStack()
{
    int threadBitmap = currentThread->getId();
    if(threadBitmap>=0 && threadBitmap<=MAX_THREAD_NUMBER)
    {
        bitMap->Clear(threadBitmap);
    } 
...
unsigned numPages){
    ...
    TranslationEntry * myOldPageTable = machine->currentPageTable ;
    int myOldPageTableSize = machine->currentPageTableSize ;
    machine->currentPageTable = pageTable ;
    machine->currentPageTableSize = numPages ;
    ...
    machine->currentPageTable = myOldPageTable ;
    machine->currentPageTableSize = myOldPageTableSize ;
